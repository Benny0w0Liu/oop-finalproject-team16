...
def run(episodes, is_training=True, render=False):
    ...
    learning_rate_a = 0.1 # alpha or learning rate 0.9
    ...
    for i in range(episodes):
        ...
        while(not terminated and not truncated):
            if is_training and rng.random() < epsilon:
                ...
                # 原本: action = np.argmax(q[state,:]) 
                # 問題: 如果 Q 值都是 0 (或相等)，argmax 永遠選動作 0 (Left)。這會導致 Agent 卡死。
                # 修正: 找出所有最大的 Q 值動作，然後隨機選一個。
                max_q_value = np.max(q[state, :])
                # 找出所有等於最大值的動作索引
                actions_with_max_q = np.where(q[state, :] == max_q_value)[0]
                # 從這些最佳動作中隨機選一個
                action = rng.choice(actions_with_max_q)
